...
client.on('guildMemberAdd', member => {
  member.guild.fetchInvites().then(guildInvites => {
    ...
    const Galal = member.guild.channels.find("name", "invite-checker");
...
client.on('message', async msg => {
  if(msg.content.startsWith('.leaveall')) {
    if(msg.author.id !== '510087749378899968') return;
...
.setURL('https://discordapp.com/oauth2/authorize?client_id=534410996823883786&scope=bot&permissions=8')
.setAuthor(client.user.username,client.user.avatarURL)
...
msg.delete();
    ...
    if (message.content === '.invite') {
           ...
           .setThumbnail(message.author.avatarURL)
           .addField("**رابط اضافه البوت**","**[https://discordapp.com/oauth2/authorize?client_id=534410996823883786&scope=bot&permissions=8] **")
    }
});
...
client.on('ready', function(){
    var ms = 10000 ;
    var setGame = [' Ahelp ','Ainvite | By -Abdo #3551 ' ];
    var i = -1;
    var j = 0;
